...
def get_user_table(ID, PW):
    ...
    if user_info is not None:  # None이 아니라는것은 아이디는 맞았을때 라는 뜻이다.
        ...
        checkpw_password = bcrypt.checkpw(PW.encode('utf-8'),
                                          hashed_password)  # 매개변수로 받은 유저가 입력한 PW를 받아 암호화된 비밀번호를  bcrypt.checkpw()를 통해 대조한다.
...
def delete_user_info(name, ID, PW):
    ...
    cursor.execute('SELECT name, login_id, password FROM user WHERE name=%s AND login_id=%s AND password=%s',
                   (name, ID, PW))
